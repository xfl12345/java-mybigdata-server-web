...
WHERE data_src_table.global_id = g.global_id
ORDER BY `g`.`global_id`;
...
(
    ...
    foreign key (global_id) references global_data_record (global_id) on delete cascade on update cascade,
    foreign key (schema_name) references global_data_record (global_id) on delete restrict on update cascade,
    unique key unique_global_id (global_id) comment '确保每一行数据对应一个相对于数据库唯一的global_id',
...
SELECT g.global_id,
       ...
       (SELECT content FROM string_content AS s WHERE s.global_id = (SELECT description FROM global_data_record AS s WHERE s.global_id = data_src_table.item_a)) AS `item_a_description`,
       data_src_table.item_b  AS `item_b`,
       (SELECT content FROM string_content AS s WHERE s.global_id = (SELECT description FROM global_data_record AS s WHERE s.global_id = data_src_table.item_b)) AS `item_b_description`
...
SELECT g.global_id,
       ...
       g.modified_count,
       (SELECT content FROM string_content AS s WHERE s.global_id = g.description) AS `description`,
       (SELECT content FROM string_content AS s WHERE s.global_id = (SELECT global_id FROM group_record AS s WHERE s.global_id = data_src_table.group_id)) AS `group_name`,
       data_src_table.item_index  AS `item_index`,
       data_src_table.item  AS `item`
FROM group_content AS data_src_table,
     global_data_record AS g
WHERE data_src_table.group_id = g.global_id
ORDER BY g.global_id;
...
BEGIN
    ...
    # 获取全局ID
    SET insert_row_gid = (SELECT global_id FROM global_data_record WHERE uuid = the_uuid);
