...
import cc.xfl12345.mybigdata.server.model.database.constant.GroupRecordConstant;
...
import cc.xfl12345.mybigdata.server.model.database.table.GroupContent;
...
public class GroupTypeHandlerImpl extends AbstractCoreTableHandler implements GroupTypeHandler {
    ...
    protected void _insertIntoGroupByGlobalId(Long globalId, Collection<Long> groupItems) throws TableOperationException {
        ...
        if (affectedRowCount != itemCount) {
            throw getAffectedRowsCountDoesNotMatch(
                affectedRowCount,
                CURD.CREATE,
                CoreTableNames.GROUP_CONTENT.getName()
            );
        }
    }
    ...
    protected Long _insertNewGroup(String name, Collection<Long> groupItems) throws Exception {
        ...
        GlobalDataRecord globalDataRecord = getNewRegisteredGlobalDataRecord(
            date,
            coreTableCache.getTableNameCache().getValue(CoreTableNames.GROUP_RECORD.getName())
        );
        ...
        checkAffectedRowShouldBe1(affectedRowCount, CURD.CREATE, CoreTableNames.GROUP_RECORD.getName());
    ...
    public void deleteGroupByGlobalId(Long globalId) throws Exception {
        ...
        checkAffectedRowShouldBe1(affectedRowCount, CURD.DELETE, CoreTableNames.GROUP_RECORD.getName());
